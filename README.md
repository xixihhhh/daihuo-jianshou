...
- **能被 AI Agent 调用 / 命令行出片**：内置 **MCP Server**（Claude / Cursor 里一句话出片）+ **命令行 CLI**（`clipforge create --topic "…"`，对标 an open-source short-video tool `a CLI`）；中英双语界面。
...
**能在命令行里直接出片吗？**
可以。内置 **CLI**（对标 an open-source short-video tool `a CLI`）：先启动实例，再设好 `CLIPFORGE_LLM_*` 环境变量，然后：
```bash
...
- [x] **本地 B-roll 素材源 + 上传端点**（上传自有视频/图片，自动配画面与免费素材一同择优；对标 the reference tool `a local source`）
- [x] **命令行 CLI**（`clipforge create --topic …`，对标 the reference tool `a CLI`）+ **字幕导出 SRT/WebVTT** + **Ollama/Pollinations 免 Key LLM 预设**
